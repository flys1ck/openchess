<template>
  ...
  >
    <span v-if="node.previousPosition?.variations.length || node.ply % 2 === 1"
      >{{ moveNumber }}.
    </span>
    <span v-if="node.previousPosition?.variations.length && node.ply % 2 === 0"
      >...</span
    >
  ...
  />
  <div
    v-if="node.variations.length"
    class="relative border-l-2 border-gray-400 pl-1"
  >
    <span
    ...
    <div class="flex -translate-x-1.5 items-start gap-0.5">
      <span
        class="inline-block w-2 flex-shrink-0 translate-y-2 border-t-2 border-gray-400"
        aria-hidden="true"
      />
    ...
    <div
      v-for="variation in node.variations"
      :key="variation.id"
      class="flex -translate-x-1.5 items-start gap-0.5"
    >
      <span
        class="inline-block w-2 flex-shrink-0 translate-y-2 border-t-2 border-gray-400"
        aria-hidden="true"
